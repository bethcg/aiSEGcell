...
def main():
    ...
    # single movies
    for movie in movies:
        tmp = data_sub.loc[data_sub.Experiment == movie, :]

        # Note: rows=gt, cols=pred
        conf_mat = confusion_matrix(tmp.loc[tmp.Set == 'gt', 'ResponseType'], tmp.loc[tmp.Set == 'pred', 'ResponseType'], labels = rts)
        ...
        conf_mat.to_csv(os.path.join(out_path, f'{movie}_confmat.csv'), sep=',')
        conf_mat_n = conf_mat / (row_sums + 1e-9)

        fig = sb.heatmap(conf_mat_n, cmap='Blues', vmin=0, vmax=1)
        plt.savefig(os.path.join(out_path, f'{movie}_confmat.pdf'), dpi=300)
        plt.close()
