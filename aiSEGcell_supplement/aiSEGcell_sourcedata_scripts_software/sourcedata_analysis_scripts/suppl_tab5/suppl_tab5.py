...
for score in scores:
    print(f'\n### Processing {score}')
    # load data
    df = pd.read_csv(f'./raw_data/suppl_tab5_{score}.csv')
    ...
    # save to csv
    mu.to_csv(os.path.join(path_out, f'{score}_mean.csv'), index=False)
    sd.to_csv(os.path.join(path_out, f'{score}_std.csv'), index=False)
