...
path_out = '../output/suppl_tab11/'
os.makedirs(path_out, exist_ok=True)

models = ['untrained', 'retrained']
...
for model in models:
    for score in scores:
        ...
        # load data
        df = pd.read_csv(f'./raw_data/suppl_tab11_{model}_{score}.csv')
        ...
        # save to csv
        mu.to_csv(os.path.join(path_out, f'{model}_{score}_mean.csv'), index=False)
        sd.to_csv(os.path.join(path_out, f'{model}_{score}_std.csv'), index=False)
